...
NOTION_ROOT = "./notion-export-2"
TARGET_DIR = "./src-2"
TITLE = "Challenge Studio 1"
...
def titleify(slug)
  slug
    ...
    .gsub(LEADING_NUMS_PATTERN, "") # replace leading nums
    .split("-").join(" ") # Go from kebab-case to title case
...
def write_summary
  File.open(File.join(TARGET_DIR, "SUMMARY.md"), "w") do |f|
    ...
    dirs.each do |dir|
      chapters = Dir.children(dir).filter { |path| path.end_with?(".md") }
...
ordered_lessons = Dir.children(NOTION_ROOT).map{ |path| File.join(NOTION_ROOT, path) }.filter { |d| Dir.exists?(d) }
ordered_lessons.map do |dir|
  puts File.join(NOTION_ROOT, dir)
  ordered_chapters = Dir.children(dir).map{ |path| File.join(dir, path) }.filter { |path| Dir.exists?(path) }.sort
  puts dir, ordered_chapters
end